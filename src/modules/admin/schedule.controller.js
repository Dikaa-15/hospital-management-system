...
async function handleCreateShift(req, res) {
  ...
  if (!doctorId || !shiftDate || !shiftType) {
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?error=Doctor%2C%20date%2C%20and%20shift%20are%20required');
  }
  ...
  try {
    ...
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Shift%20created');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
...
async function handleDeleteShift(req, res) {
  try {
    ...
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Shift%20deleted');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
...
async function handleCreateRoom(req, res) {
  ...
  if (!roomCode || !roomClass) {
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?error=Room%20code%20and%20class%20are%20required');
  }
  ...
  try {
    ...
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Room%20created');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
...
async function handleRoomStatus(req, res) {
  try {
    ...
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Room%20updated');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
...
async function handleCreateTransfer(req, res) {
  ...
  if (!patientName || !fromUnit) {
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?error=Patient%20name%20and%20origin%20unit%20are%20required');
  }
  ...
  try {
    ...
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Transfer%20request%20created');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
...
async function handleTransferStatus(req, res) {
  ...
  if (!allowed.has(status)) {
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?error=Invalid%20transfer%20status');
  }

  try {
    await updateTransferStatus(req.params.id, status);
    return res.redirect('/templates/dashboard-admin/schedule-ward-management?flash=Transfer%20status%20updated');
  } catch (error) {
    return res.redirect(`/templates/dashboard-admin/schedule-ward-management?error=${encodeURIComponent(error.message)}`);
  }
}
